...
def loose_json_loads(s: str) -> Any:
    """Best-effort 'JSON-ish' parser."""
    if not isinstance(s, str):
        s = str(s)
    s = _strip_bom_zw(_normalize_quotes(_strip_comments(_strip_trailing_commas(s)))).strip()
    try:
        return json.loads(s)
    except Exception:
        pass
...
def force_decode_any(x: Any) -> Any:
    """Accept dict/list; if str/bytes, attempt JSON or base64->JSON."""
...
def looks_like_figma(obj: Any) -> bool:
    """Loose Figma-ish detector covering common node shapes and plugin exports."""
    try:
        for n in _walk(obj):
...
def _header_likeliness(s: str) -> float:
    s = s.strip()
    if not s or _is_numbery(s) or len(s) < 2 or len(s) > 60:
        return 0.0
    toks = _tokens(s)
...
def extract_figma_headers(figma: Dict[str, Any]) -> List[str]:
    ...
    Harvest headers from:
      - TEXT.characters / name / title / headerName / field
      - columns[] entries
      - keys themselves (split camelCase & delimiters)
      - JSON-in-string values (e.g., frameMeta)
      - then drop junky labels and prefer longer phrases
    """
    cand: List[str] = []
    ...
    def consider(val: Any):
        if isinstance(val, str):
            s = val.strip()
            # parse JSON-in-string (e.g., frameMeta)
            ...
            for col in cols:
                if isinstance(col, dict):
                    for kk in ("header","headerName","name","title"):
                        if kk in col: consider(col.get(kk))
                elif isinstance(col, str):
                    consider(col)

        # 3) mine *keys* themselves
        for k in list(node.keys()):
            if not isinstance(k, str): continue
            if k in {"id","type","class","href","url","key"}: continue
    ...
    # rank & cap
    scored = sorted(((s, _header_likeliness(s)) for s in uniq), key=lambda t: t[1], reverse=True)
...
def synonym_boost(hdr: str, fld: str) -> float:
    h = " ".join(_tokens(hdr)); f = " ".join(_tokens(fld))
    b = 0.0
...
# =========================== Optional classic LLM rerank (kept) ===========================
...
# =========================== LLM-first extraction with ID protocol ===========================
...
# =========================== Body scanning & explicit preference ===========================
...
def _headers_map_core():
    try:
        ...
        # candidates
        figma_candidates = extract_figma_headers(figma)
        schema_fields = collect_schema_fields(schema)

        # heuristic seeds (hints for LLM)
        seeds = {h: [f for f in top_k_fields(h, schema_fields, k=3)] for h in figma_candidates}

        # optional: hard fail if require_llm and server unreachable
        used_llm = False
        result = None
        if use_llm:
